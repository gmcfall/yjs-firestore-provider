    ...
    constructor(firebaseApp, ydoc, path, config) {
        ...
        this.updateHandler = (update, origin) => {
            ...
            if (origin !== self) {
                ...
                self.cache = self.cache ? Y$2.mergeUpdates([self.cache, update]) : update;
                self.updateCount++;
        ...
        Kl(baseRef).then(baseDoc => {
            if (baseDoc.exists()) {
                ...
                Y$2.applyUpdate(ydoc, update, self);
            }
        }).then(() => {
            ...
            for (const [key, value] of map) {
                if (value) {
                    ...
                    if (now - value.time > this.blobTimeToLive) {
                        zombies.add(key);
                        newUpdates = newUpdates ? Y$2.mergeUpdates([newUpdates, update]) : update;
                    }
                }
            }
            if (!newUpdates) {
            ...
            try {
                ...
                yield lf(db, (txn) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    if (baselineDoc.exists()) {
                        ...
                        update = Y$2.mergeUpdates([baselineData.update.toUint8Array(), newUpdates]);
                    }
                    else {
                        update = newUpdates;
                    }
